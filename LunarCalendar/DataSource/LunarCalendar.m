...
+ (void)initStaticDataSource
{
    ...
    arrayChineseDay = @[@"春节",@"元宵节",@"端午节",@"七夕节",@"中秋节",@"重阳节"];
}
...
-(void)loadWithDate:(NSDate *)adate
{
    ...
    NSDateFormatter *dateFormatter = [Datetime staticFormatter];//[[NSDateFormatter alloc] init];
...
-(void)InitializeValue
{
	...
	NSDateFormatter *f = [Datetime staticFormatter];//[[NSDateFormatter alloc] init];
	...
	//计算闰月
	doubleMonth = [self leapMonth:lunarYear];
	isLeap = false;
	...
	for (lunarMonth = 1; lunarMonth < 13 && sumdays > 0; lunarMonth++)
	{
		...
		if (doubleMonth > 0 && lunarMonth == (doubleMonth + 1) && isLeap == false)
		{
			--lunarMonth;
			isLeap = true;
			tempdays = [self leapDays:lunarYear];
		}
		else
		{
			tempdays = [self monthDaysOfYear:lunarYear andMonth:lunarMonth];
		}
	...
	for (int i=0; i<2; i++)
	{
		NSDateFormatter *currentFormatter = [Datetime staticFormatter];//[[NSDateFormatter alloc] init];
...
- (int)leapDays:(int) y
{
    ...
    {
        if ((LunarCalendarInfo[y - 1900] & 0x10000) != 0)
            return 30;
        else
            return 29;
    }
...
-(int)leapMonth:(int) y
{
    return (int)(LunarCalendarInfo[y - 1900] & 0xf);
}

//传回农历 y年m月的总天数
-(int)monthDaysOfYear:(int) y andMonth:(int) m
{
    if ((LunarCalendarInfo[y - 1900] & (0x10000 >> m)) == 0)
        return 29;
    else
        return 30;
}
...
- (NSString* )ChineseStatutoryHolidays
{
    /*
     1.1 元旦
     春节
...
-(int)GregorianYear
{
	NSDateFormatter *formatter = [Datetime staticFormatter];//[[NSDateFormatter alloc] init];
