...
//base of all year
#define ALL_YEARS_RELATE

// Constants times :
#define kFlatDatePickerAnimationDuration 0.3

// Constants colors :

//选择器的背景色（分割线可见）
#define kFlatDatePickerBackgroundColor [UIColor colorWithRed:0xda/255.0 green:0xda/255.0 blue:0xda/255.0 alpha:1.0]

//头部背景色
#define kFlatDatePickerBackgroundColorTitle [UIColor colorWithRed:0xff/255.0 green:0xff/255.0 blue:0xff/255.0 alpha:1.0]

//确定按钮背景色
#define kFlatDatePickerBackgroundColorButtonValid [UIColor colorWithRed:0x00/255.0 green:0x7a/255.0 blue:0xff/255.0 alpha:1.0]

//取消按钮背景色
#define kFlatDatePickerBackgroundColorButtonCancel [UIColor colorWithRed:0xff/255.0 green:0x77/255.0 blue:0x00/255.0 alpha:1.0]

//ScrollView 背景色
#define kFlatDatePickerBackgroundColorScrolView [UIColor colorWithRed:0xff/255.0 green:0xff/255.0 blue:0xff/255.0 alpha:1.0]

//选中日期横线
#define kFlatDatePickerBackgroundColorLines [UIColor colorWithRed:51.0/255.0 green:181.0/255.0 blue:229.0/255.0 alpha:0.0]
#define kCOOLLinesBackgroundColor [UIColor colorWithRed:51.0/255.0 green:181.0/255.0 blue:229.0/255.0 alpha:1.0]

// Constants fonts colors :
//头字体颜色
#define kFlatDatePickerFontColorTitle [UIColor colorWithRed:0x33/255.0 green:0x33/255.0 blue:0x33/255.0 alpha:0.8]

//非选中日期字体颜色
#define kFlatDatePickerFontColorLabel [UIColor colorWithRed:0xa6/255.0 green:0xa6/255.0 blue:0xa6/255.0 alpha:0.65]

//选中字体颜色
#define kFlatDatePickerFontColorLabelSelected [UIColor colorWithRed:0x33/255.0 green:0x33/255.0 blue:0x33/255.0 alpha:0.5]
...
// Constants :
#define kStartYear 1901
#define kMaxYear 2099
#define TAG_DAYS 1
#define TAG_MONTHS 2
#define TAG_YEARS 3
#define TAG_HOURS 4
...
-(id)initWithParentView:(UIView*)parentView {
    ...
    if (self = [super init]) {
        [self setFrame:CGRectMake(0.0, _parentView.frame.size.height, _parentView.frame.size.width, kFlatDatePickerHeight)];
        _datePickerMode = FlatDatePickerModeDate;
...
-(void)setupControl {
    ...
    if (self.datePickerMode == FlatDatePickerModeMonthYear) {
        _years = [self getYears];
        _months = [self getMonths];

        [self buildSelectorDaysOffsetX:0.0 andWidth:0.0];
        [self buildSelectorMonthsOffsetX:0.0 andWidth:((self.frame.size.width / 2.0) - kFlatDatePickerScrollViewLeftMargin)];
        [self buildSelectorYearsOffsetX:((self.frame.size.width / 2.0)) andWidth:(self.frame.size.width / 2.0)];
    }
...
- (void)buildHeaderWithOutTitle
{
    // Button Cancel
//    _buttonClose = [[UIButton alloc] initWithFrame:CGRectMake(0.0, 0.0, SCREEN_WIDTH/2.0f, kFlatDatePickerHeaderHeight)];
...
//    [_buttonClose addTarget:self action:@selector(actionButtonCancel) forControlEvents:UIControlEventTouchUpInside];
    ...
    _buttonClose = [FlatDatePicker commonRedButtonForTitle:@"取消" target:self action:@selector(actionButtonCancel)];
    [_buttonClose setFrame:CGRectMake(0.0, 1.0, SCREEN_WIDTH/2.0f-1, kFlatDatePickerHeaderHeight - 1)];
    [self addSubview:_buttonClose];
    
    // Button Valid
//    _buttonValid = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2.0f, 0.0,  SCREEN_WIDTH/2.0f, kFlatDatePickerHeaderHeight)];
...
//    [_buttonValid addTarget:self action:@selector(actionButtonValid) forControlEvents:UIControlEventTouchUpInside];
    _buttonValid = [FlatDatePicker commonBlueButtonForTitle:@"确定" target:self action:@selector(actionButtonValid)];
    [_buttonValid setFrame:CGRectMake(SCREEN_WIDTH/2.0f, 1.0, SCREEN_WIDTH/2.0f, kFlatDatePickerHeaderHeight - 1)];
...
- (void)buildHeader {
    ...
    // Button Valid
    _buttonValid = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - kFlatDatePickerButtonHeaderWidth, 0.0, kFlatDatePickerButtonHeaderWidth, kFlatDatePickerHeaderHeight)];
    _buttonValid.backgroundColor = kFlatDatePickerBackgroundColorButtonValid;
    [_buttonValid setImage:[UIImage imageNamed:kFlatDatePickerIconValid] forState:UIControlStateNormal];
    ...
    // Label Title
    _labelTitle = [[UILabel alloc] initWithFrame:CGRectMake(kFlatDatePickerButtonHeaderWidth, 0.0, SCREEN_WIDTH - (kFlatDatePickerButtonHeaderWidth * 2), kFlatDatePickerHeaderHeight)];
    ...
    if (self.datePickerMode != FlatDatePickerModeMonthYear) {
        UITapGestureRecognizer *singleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(singleTapGestureHeaderToChangeSelectType:)];
        singleTap.numberOfTapsRequired = 1;
        singleTap.numberOfTouchesRequired = 1;
        [_labelTitle addGestureRecognizer:singleTap];
...
- (void)buildSelectorDaysOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    _lineDaysTop = [[UIView alloc] initWithFrame:CGRectMake(_scollViewDays.frame.origin.x + kFlatDatePickerLineMargin, _scollViewDays.frame.origin.y + (_scollViewDays.frame.size.height / 2) - (kFlatDatePickerScrollViewItemHeight / 2), MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
    _lineDaysTop.backgroundColor = kCOOLLinesBackgroundColor;
    [self addSubview:_lineDaysTop];
    
    _lineDaysBottom = [[UIView alloc] initWithFrame:CGRectMake(_scollViewDays.frame.origin.x + kFlatDatePickerLineMargin, _scollViewDays.frame.origin.y + (_scollViewDays.frame.size.height / 2) + (kFlatDatePickerScrollViewItemHeight / 2), MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
    _lineDaysBottom.backgroundColor = kCOOLLinesBackgroundColor;
...
- (void)buildSelectorMonthsOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    _lineMonthsTop = [[UIView alloc] initWithFrame:CGRectMake(_scollViewMonths.frame.origin.x + kFlatDatePickerLineMargin, _scollViewMonths.frame.origin.y + (_scollViewMonths.frame.size.height / 2) - (kFlatDatePickerScrollViewItemHeight / 2), MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
    _lineMonthsTop.backgroundColor = kCOOLLinesBackgroundColor;
    [self addSubview:_lineMonthsTop];
    
    _lineMonthsBottom = [[UIView alloc] initWithFrame:CGRectMake(_scollViewMonths.frame.origin.x + kFlatDatePickerLineMargin, _scollViewMonths.frame.origin.y + (_scollViewMonths.frame.size.height / 2) + (kFlatDatePickerScrollViewItemHeight / 2), MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
    _lineMonthsBottom.backgroundColor = kCOOLLinesBackgroundColor;
...
- (void)buildSelectorYearsOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    _lineYearsTop = [[UIView alloc] initWithFrame:CGRectMake(_scollViewYears.frame.origin.x + kFlatDatePickerLineMargin, _scollViewYears.frame.origin.y + (_scollViewYears.frame.size.height / 2) - (kFlatDatePickerScrollViewItemHeight / 2), MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
    _lineYearsTop.backgroundColor = kCOOLLinesBackgroundColor;
    [self addSubview:_lineYearsTop];
    
    _lineYearsBottom = [[UIView alloc] initWithFrame:CGRectMake(_scollViewYears.frame.origin.x + kFlatDatePickerLineMargin, _scollViewYears.frame.origin.y + (_scollViewYears.frame.size.height / 2) + (kFlatDatePickerScrollViewItemHeight / 2), MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
    _lineYearsBottom.backgroundColor = kCOOLLinesBackgroundColor;
...
- (void)buildSelectorDatesOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    _lineDatesTop = [[UIView alloc] initWithFrame:CGRectMake(_scollViewDates.frame.origin.x + kFlatDatePickerLineMargin, _scollViewDates.frame.origin.y + (_scollViewDates.frame.size.height / 2) - (kFlatDatePickerScrollViewItemHeight / 2),MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
    _lineDatesTop.backgroundColor = kFlatDatePickerBackgroundColorLines;
    [self addSubview:_lineDatesTop];
    
    _lineDatesBottom = [[UIView alloc] initWithFrame:CGRectMake(_scollViewDates.frame.origin.x + kFlatDatePickerLineMargin, _scollViewDates.frame.origin.y + (_scollViewDates.frame.size.height / 2) + (kFlatDatePickerScrollViewItemHeight / 2), MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
...
- (void)buildSelectorLabelsDates {
    ...
    _lineDatesBottom.hidden = YES;
    _lineDatesTop.hidden = YES;
}
...
- (void)buildSelectorHoursOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    _lineHoursTop = [[UIView alloc] initWithFrame:CGRectMake(_scollViewHours.frame.origin.x + kFlatDatePickerLineMargin, _scollViewHours.frame.origin.y + (_scollViewHours.frame.size.height / 2) - (kFlatDatePickerScrollViewItemHeight / 2), MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
    _lineHoursTop.backgroundColor = kFlatDatePickerBackgroundColorLines;
    [self addSubview:_lineHoursTop];
    
    _lineHoursBottom = [[UIView alloc] initWithFrame:CGRectMake(_scollViewHours.frame.origin.x + kFlatDatePickerLineMargin, _scollViewHours.frame.origin.y + (_scollViewHours.frame.size.height / 2) + (kFlatDatePickerScrollViewItemHeight / 2), MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
...
- (void)buildSelectorMinutesOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    _lineMinutesTop = [[UIView alloc] initWithFrame:CGRectMake(_scollViewMinutes.frame.origin.x + kFlatDatePickerLineMargin, _scollViewMinutes.frame.origin.y + (_scollViewMinutes.frame.size.height / 2) - (kFlatDatePickerScrollViewItemHeight / 2), MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
    _lineMinutesTop.backgroundColor = kFlatDatePickerBackgroundColorLines;
    [self addSubview:_lineMinutesTop];
    
    _lineMinutesBottom = [[UIView alloc] initWithFrame:CGRectMake(_scollViewMinutes.frame.origin.x + kFlatDatePickerLineMargin, _scollViewMinutes.frame.origin.y + (_scollViewMinutes.frame.size.height / 2) + (kFlatDatePickerScrollViewItemHeight / 2), MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
...
- (void)buildSelectorSecondsOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    _lineSecondsTop = [[UIView alloc] initWithFrame:CGRectMake(_scollViewSeconds.frame.origin.x + kFlatDatePickerLineMargin, _scollViewSeconds.frame.origin.y + (_scollViewSeconds.frame.size.height / 2) - (kFlatDatePickerScrollViewItemHeight / 2), MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
    _lineSecondsTop.backgroundColor = kFlatDatePickerBackgroundColorLines;
    [self addSubview:_lineSecondsTop];
    
    _lineSecondsBottom = [[UIView alloc] initWithFrame:CGRectMake(_scollViewSeconds.frame.origin.x + kFlatDatePickerLineMargin, _scollViewSeconds.frame.origin.y + (_scollViewSeconds.frame.size.height / 2) + (kFlatDatePickerScrollViewItemHeight / 2), MAX(0,width - (2 * kFlatDatePickerLineMargin)), kFlatDatePickerLineWidth)];
...
- (NSDate*)convertToDateDay:(NSInteger)day month:(NSInteger)month year:(NSInteger)year hours:(NSInteger)hours minutes:(NSInteger)minutes seconds:(NSInteger)seconds date:(NSDate*)date; {
    ...
    // Date Mode : Month Year Mode
    if (self.datePickerMode == FlatDatePickerModeDate || self.datePickerMode == FlatDatePickerModeMonthYear) {
...
#pragma mark -Tap header to change select type
- (void)singleTapGestureHeaderToChangeSelectType:(UIGestureRecognizer*)gesture
{
    if (gesture.state == UIGestureRecognizerStateEnded) {
        switch (self.datePickerMode) {
            case FlatDatePickerModeDate:
            {
                [self setDatePickerMode:FlatDatePickerModeMonthYear];
            }
                break;
            case FlatDatePickerModeMonthYear:
            {
                [self setDatePickerMode:FlatDatePickerModeDate];
            }
                break;
            default:
                break;
...
//    button.frame = CGRectMake(0, 0, 320 - 20, 44);
//    CALayer * downButtonLayer = [button layer];
//    [downButtonLayer setMasksToBounds:YES];
//    [downButtonLayer setCornerRadius:6.0];
//    [downButtonLayer setBorderWidth:1.0f];
//    [downButtonLayer setBorderColor:[[UIColor colorWithRed:0xa6/255.0 green:0xa6/255.0 blue:0xa6/255.0 alpha:1] CGColor]];
    
    UIImage *bgImage = [UIImage imageWithColor:RGBACOLOR(0xf7,0xf7,0xf7,1.0f)];
    [button setBackgroundImage:bgImage forState:UIControlStateNormal];
    
    UIImage *hImage = [UIImage imageWithColor:RGBACOLOR(0x00, 0x6a, 0xc1, 1.0f)];
    [button setBackgroundImage:hImage forState:UIControlStateHighlighted];
    
    UIImage *dImage = [UIImage imageWithColor:RGBACOLOR(0xa6, 0xa6, 0xa6, 1.0f)];
    [button setBackgroundImage:dImage forState:UIControlStateDisabled];
    
    button.titleLabel.font = [UIFont systemFontOfSize:17];
    [button setTitle:title forState:UIControlStateNormal];
    [button setTitleColor:RGBACOLOR(0x00, 0x6a, 0xc1, 1) forState:UIControlStateNormal];
    [button setTitleColor:RGBACOLOR(0xff, 0xff, 0xff, 1) forState:UIControlStateHighlighted];
    [button setTitleShadowColor:RGBACOLOR(0, 0, 0, 0.3f) forState:UIControlStateHighlighted];
...
+ (UIButton*)commonRedButtonForTitle:(NSString*)title target:(id)target action:(SEL)action
{
    UIButton* button = [UIButton buttonWithType:UIButtonTypeCustom];
    ...
    UIImage *hImage = [UIImage imageWithColor:RGBACOLOR(0xfc, 0x42, 0x26, 1.0f)];
    [button setBackgroundImage:hImage forState:UIControlStateHighlighted];
    ...
    button.titleLabel.font = [UIFont systemFontOfSize:17];
    [button setTitle:title forState:UIControlStateNormal];
    [button setTitleColor:RGBACOLOR(0xfc, 0x42, 0x26, 1) forState:UIControlStateNormal];
    [button setTitleColor:RGBACOLOR(0xff, 0xff, 0xff, 1) forState:UIControlStateHighlighted];
    [button setTitleShadowColor:RGBACOLOR(0, 0, 0, 0.3f) forState:UIControlStateHighlighted];
    button.titleLabel.shadowOffset = CGSizeMake(0, 1);
    [button addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
